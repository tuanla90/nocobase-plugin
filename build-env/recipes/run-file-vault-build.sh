...
PKG="@tuanla90/plugin-file-vault"
echo "node: $(node -v)"

# Stage source + lane markers from the workspace (source of truth).
SRC="$ROOT/../packages/@tuanla90/plugin-file-vault"
DST="$ROOT/packages/plugins/@tuanla90/plugin-file-vault"
...
# resolvable package in node_modules — refreshed by run-shared-build.sh), same as ip-guard/plugin-hub.
mkstub() {
...
find "$ROOT/storage/tar" -type f -name "*plugin-file-vault*" 2>/dev/null || true
