...
PKG="@ptdl/plugin-change-log"
echo "node: $(node -v)"

# Stage source + markers (markers at the STAGED package root are the lane build triggers!)
SRC="$ROOT/../packages/@ptdl/plugin-change-log"
DST="$ROOT/packages/plugins/@ptdl/plugin-change-log"
...
echo "=== TAR OUTPUT ==="
TGZ=$(find "$ROOT/storage/tar" -type f -name "*plugin-change-log*" 2>/dev/null | head -1)
echo "$TGZ"
if [ -n "$TGZ" ]; then
