#!/usr/bin/env bash
# Build @ptdl/shared (src -> dist CJS+ESM+dts) and sync into build-env/node_modules
# so consumer plugin builds resolve+bundle the fresh version.
set -e
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"
SHARED="$ROOT/../packages/@ptdl/shared"
BIN="$ROOT/node_modules/.bin"

echo "=== build @ptdl/shared ==="
...
# Pure subpath: `@ptdl/shared/format` (no react/antd) — for util-only consumers (data-viz, enhanced-table)
...
# Server-only subpath: `@ptdl/shared/ai-server` (no react/antd) — the AI codegen helper a plugin's
...
echo "=== sync -> build-env/node_modules/@ptdl/shared ==="
rm -rf "$ROOT/node_modules/@ptdl/shared/dist"
mkdir -p "$ROOT/node_modules/@ptdl/shared"
cp -r "$SHARED/dist" "$SHARED/package.json" "$ROOT/node_modules/@ptdl/shared/"
