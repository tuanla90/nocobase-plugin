#!/usr/bin/env node
/**
 * Regenerate the two catalog docs the plugin distribution relies on, both from `latest/@ptdl/*.tgz`
 * (single source of truth for shipped versions). Run after every rebuild that refreshes latest/.
 *
 *   node build-env/gen-manifest.cjs
 *
 *   1. latest/index.json — the machine manifest @ptdl/plugin-hub reads ({packageName,version,url}).
...
// 1) manifest
const manifest = { source: '@ptdl/nocobase-plugin', updatedAt: null, count: plugins.length, plugins: plugins.map(({ file, ...p }) => p) };
...
## Danh sách (${plugins.length} plugin — sync theo \`latest/@ptdl/\`)
