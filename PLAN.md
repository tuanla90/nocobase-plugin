...
|---|--------|---------|:---:|:---:|:---:|:---:|
| 1 | Custom Header ✅ | `@tuanla90/plugin-custom-header` | ✅ | ✅ | Thấp | Cao |
| 2 | Spreadsheet View | `@tuanla90/plugin-spreadsheet-view` | ✅ | ⚠️ (bulk API) | **Cao** | Trung |
| 3 | Table Cell Display (RunJS) 🟢 snippet | `@tuanla90/plugin-table-cell-display` | ✅ | – | Trung | Cao |
| 4 | Status Flow Column | `@tuanla90/plugin-status-flow` | ✅ | ✅ | **Cao** | Cao |
| 5 | QR Scanner | `@tuanla90/plugin-qr-scanner` | ✅ | – | Thấp–Trung | Trung |
| 6 | Default Value Expression | `@tuanla90/plugin-default-value-expression` | ✅ | ⚠️ | Trung | Trung |
| 7 | Computed / Virtual Field | `@tuanla90/plugin-computed-field` (+ core formula cho case đơn giản) | ✅ | ✅ | **Cao** | Cao |
| 8 | Rich Dropdown Picker (+ thống kê) 🟢 snippet | `@tuanla90/plugin-rich-select` | ✅ | ⚠️ (count) | Trung | Trung |
| 9 | Input Prefix Icon 🟢 snippet | `@tuanla90/plugin-input-icon` | ✅ | – | Thấp | Trung |
| 10 | Button Icon Picker | `@tuanla90/plugin-button-icon` | ✅ | – | Thấp | Trung |
| 11 | Field Widgets (checkbox→toggle, multi-select→button) 🟢 snippet | `@tuanla90/plugin-field-widgets` | ✅ | – | Thấp | Trung |
| 12 | Menu Sections (group title + divider) ✅ | `@tuanla90/plugin-menu-sections` | ✅ | ✅(no-op) | Trung | – |
| 13 | Icon Remap (system icon → Lucide) ✅ | `@tuanla90/plugin-icon-remap` | ✅ | ✅ | Trung | – |
| 14 | Menu Count Badge ✅ | `@tuanla90/plugin-menu-badge` | ✅ | ✅(no-op) | Trung | – |
| 15 | **Field Enhancements** (gom snippet field #8+#9+#11 + display) ✅ snippet-lib | `@tuanla90/plugin-field-enhancements` | ✅ | – | Trung | **Cao** |
| 16 | Filter Tree (AppSheet group-by block) 🔨 MVP | `@tuanla90/plugin-filter-tree` | ✅ (v2 only) | ✅(no-op) | **Cao** | – |
| 17 | **Sub-table Pro** (widget hasMany 3 tầng: standalone → view/tổng → bridge nhận event từ table khác) ✅ v0.2.0 | `@tuanla90/plugin-subtable-pro` | ✅ | – | Trung | Cao |

> **Chú thích trạng thái:** ✅ = plugin đã build+cài xong · 🟢 snippet = đã có bản RunJS dùng được ngay (chưa plugin hoá) · 🔨 = đang thiết kế/dựng.
>
> #17 Sub-table Pro: **SHIPPED v0.2.0** (2026-07-15) — v0 standalone + v1 view/tổng/stepper + v2 bridge (+ no-code publish action/rowClick, membership sync, tabbed config) đều xong & verified live. Thiết kế/trạng thái: `docs/SUBTABLE-PRO-DESIGN.md`; hướng dẫn: `packages/@tuanla90/plugin-subtable-pro/README.md`. Còn lại: m2m through-qty, multi-device WS.

**Nhóm ICON (#1, #9, #10):** đều cần một component **`IconPicker` dùng chung** dựa trên registry của `@tuanla90/plugin-custom-icons`. → **Làm IconPicker 1 lần** (đặt trong custom-icons hoặc 1 shared lib), 3 plugin kia tái dùng. Có thể gộp #9+#10 thành 1 plugin `@tuanla90/plugin-ui-icons` nếu muốn gọn.
...
> - **Deploy (đã làm, LIVE):** extract `latest/@tuanla90/plugin-filter-tree-0.2.6.tgz` vào **cả** `nb-local/node_modules/@tuanla90/plugin-filter-tree` **và** `storage/plugins/...` (đè dist+markers) → `npx pm2 restart index` (từ nb-local) → F5 (hash `?hash=sha256(mtime+ver)` đổi → tự bust cache). Bundle client-v2 = 68.6 KB. Update plugin đã cài — KHÔNG đụng DB.
...
>   → hiện trong settings "Field component" cho user chọn (no-code). Pattern chuẩn workspace: `@tuanla90/plugin-conditional-format`
...
> - Cài: giải nén tgz → `nb-local/node_modules/@tuanla90/plugin-field-enhancements` (tar `--force-local` vì path D:) + copy root
>   stubs client.js/client-v2.js/server.js (tgz KHÔNG chứa, nhưng runtime cần để Node resolve `@tuanla90/.../client`) → INSERT
...
> **Cấu trúc:**
> ```
> @tuanla90/plugin-field-enhancements
>  ├─ shared/    IconPicker (consumer @tuanla90/plugin-custom-icons) · colorPreset antd 12 màu · ctxHelpers (form/table detect, association hydrate, api list)
...
> (formula ĐÃ tách riêng `@tuanla90/plugin-formula` — dep formulajs nặng + hướng default-value server; KHÔNG gộp vào đây.)
>
...
>
> **Build/cài:** `build-env/recipes/run-menu-sections-build.sh` → `add-markers.sh` → giải nén vào `nb-local/node_modules/@tuanla90/`
> (tar cần `--force-local` vì path `D:`), INSERT applicationPlugins (id 90, enabled+installed), `yarn start`.

---

## 1. Custom Header
> **Trạng thái (2026-07-13): ✅ v0.2.0 — nghiệm thu PASS + đã deploy nb-local (node_modules/@tuanla90, pm2 `index`).** ✅ Polish UI config: gom field vào `CollapsibleSection` (Icon/Text/Background/Alignment) + preview dùng `PreviewBox` — chuẩn `@tuanla90/shared` như layout-containers (chỉ đổi trình bày, giữ key/handler). Đã deploy nb-local. Thêm **Size** cho Column/Form-Detail label/Block (trước chỉ page); **tách Header align vs Cell align** (cell = antd `align`, header override qua `onHeaderCell`/`chHeaderAlign`); **JS Field trong form** (`FormJSFieldItemModel` → flow `ptdlJsFieldLabel`, resolve theo tên); **cache auto-refresh** khi tab refocus (`bindFieldStyleAutoRefresh`, throttle 10s; view đang mở cần re-render kế tiếp); **i18n** en/vi/zh (`addResources` ns `@tuanla90/plugin-custom-header/client`; `t()` sinh expression có `ns` — pattern enhanced-table; nhãn React Segmented/Preview dịch runtime qua `setRuntimeT`+app.i18n); **gradient nền** 2 màu+hướng cho page & block (`headerBg2`+`bgDirection`→`linear-gradient`, component `ChBgDir`; clear màu = xóa nền sạch); **xóa dead code** `registerTabStyle`; guard `pageSize:1000`; thêm `README.md`. Checklist + test nghiệm thu: `packages/@tuanla90/plugin-custom-header/IMPROVEMENTS-CHECKLIST.md`. Build OK → `storage/tar/@tuanla90/plugin-custom-header-0.2.0.tgz` (chưa promote lên `latest/`).
...
**Hướng kỹ thuật:** **client-only**. Thêm mục vào `SchemaSettings` của page/block title → lưu style vào `x-component-props`/`x-decorator-props`. Override component render tiêu đề để đọc prop style mới. Icon dùng lại registry của `@tuanla90/plugin-custom-icons`.
...
>
> *(Spike gốc)* `@tuanla90/plugin-spreadsheet-view`
...
> tự code editors cho AG Grid → widget @tuanla90 (Star/Progress/RichSelect…) tự chạy trong cell; formula view dùng ptdlFormula
...
## 5. QR Scanner
> **Trạng thái (2026-07-16): 🔨 P1a ĐÃ BUILD v0.1.0 — `@tuanla90/plugin-device-kit`.** Camera widget (chụp tại chỗ
> getUserMedia, **watermark ON** giờ/GPS/user burn pixel, nén, meta tự lưu vào field) + field type GPS `ptdlLocation`
> (json, no map key, hiện trong Add field nhóm "Thiết bị"). tgz `latest/@tuanla90/plugin-device-kit-0.1.0.tgz` → upload
...
**Tính năng:** nút/scan action mở camera → quét → điền field hoặc lookup bản ghi theo mã; chạy tốt trên mobile (đã có `@tuanla90/plugin-pwa`).
...
contract lập trình cho plugin khác đọc qua registry). `@tuanla90/plugin-icon-kit` (provider gốc trong
...
>   `@tuanla90/plugin-custom-icons` (provider icon đang active) pin — xem memory `nocobase-icon-architecture` về vì
...
## Ghi chú chung
- Mỗi plugin: source ở `packages/@tuanla90/plugin-<name>/` (client/, server/, locale/), recipe build ở `build-env/recipes/`.
...
- **Trạng thái tổng (cập nhật 2026-07-17):** gần như TẤT CẢ mục dưới đã plugin-hoá & LIVE. Nguồn chuẩn = `PLUGIN-REGISTRY.md` (28 plugin @tuanla90). Bảng #-số dưới đây là kế hoạch GỐC — ánh xạ sang plugin thực tế:
