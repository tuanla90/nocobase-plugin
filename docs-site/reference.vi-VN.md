...
| Text | `CONCATENATE` · `LEFT` · `RIGHT` · `MID` · `UPPER` · `LOWER` · `TRIM` · `LEN` · `TEXT` · `SPLIT` · `TEXTJOIN(sep, bỏ_ô_trống, …)` · `CONTAINS` · `STARTSWITH` · `ENDSWITH` · `INITIALS(tên)` |
...
| Danh sách / mảng | `LIST(a, b, …)` tạo mảng · `UNIQUE(mảng)` / `DISTINCT` lọc trùng · `INTERSECT(a, b)` giao 2 mảng · `ANY(mảng)` phần tử đầu · `IN(x, mảng)` có thuộc? · `SPLIT(text, sep)` — đếm phần tử bằng `COUNTA` |
| Ngày | `TODAY` · `NOW` · `YEAR` · `MONTH` · `DAY` · `DATEDIF` · `EDATE` · `DAYS` · `ADDDAYS(date, n)` · `ADDMONTHS(date, n)` · `ADDYEARS(date, n)` · `DATEADD(date, n, "day"/"month"/"year")` — `n` âm để **trừ** |
