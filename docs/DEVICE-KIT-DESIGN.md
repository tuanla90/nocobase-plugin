...
| **v0.2–0.4 (vượt plan, 2026-07-16)** | ✅ Bản đồ **Leaflet tương tác** (OSM, no key) — pick-on-map lúc nhập (plan chỉ có iframe OSM); ✅ **Auto ghi nhận khi Lưu** (form-level enable + field options: location timing/required, camera require-photo) — thay hướng "auto khi mở form"; ✅ **Device-info capture** (OS/browser/model/screen/pseudo-id, ghi khi submit) — ngoài plan; ✅ **Permission guidance** component (android/ios/desktop + Thử lại); ✅ fix crash useForm; ✅ UI polish. | done |
| **P1b ✅ BUILT v0.5.0** | ✅ **A1** scan-input widget (📷 → decode → field value; beep/rung/regex/auto-submit) · ✅ **A2** scan→lookup collection action (tra field trong bảng → **thêm vào giỏ Sub-table Pro** qua `globalThis.__ptdlBridge` POS continuous / lọc bảng / thông báo) · ✅ **A3** QR display (antd `<QRCode>`, template `{{cột}}` + deep-link). Lib `html5-qrcode` bundle tĩnh vào index.js (676KB). scanModal dùng chung + PermissionHelp. | done |
| **C2 ✅ BUILT v0.5.0** | ✅ Nút **"Check-in (vị trí)"** = record action (`scene='record'`): 1-chạm getCurrentFix → `<collection>:update` field Vị trí của dòng đó → refresh; option độ-chính-xác-cao / hỏi-xác-nhận / nhãn; denied → Modal PermissionHelp. | done |
| **Chữ ký ✅ BUILT v0.6.0** | ✅ **Signature pad** — `PtdlSignatureFieldModel extends UploadFieldModel` (như camera); nút "✍️ Ký" → canvas pointer (chuột+chạm) → PNG → `attachments:create` → append field đính kèm. Config: màu/nét bút, nền trắng/trong, chiều cao, **đóng dấu tên+giờ dưới chữ ký**. `signaturePad.tsx` + `signatureFieldModel.tsx`. | done |
| **Reverse geocode ✅ BUILT v0.7.0** | ✅ Toạ độ → **địa chỉ chữ** trên field Vị trí. `geocode.ts` fetch **Nominatim (OSM, miễn phí, không key, CORS mở)** + throttle module-level ≤1 req/giây (policy); parse structured → địa chỉ gọn VN, ghi `address` vào json. Config `addrMode` off/manual(nút 🏠)/auto(sau khi định vị) + `addrLang` (vi). Hiện dưới toạ độ ở cả input + display. | done |
| **Ghi âm ✅ BUILT v0.8.0** | ✅ **Audio recorder** — `PtdlAudioFieldModel extends UploadFieldModel`; nút "🎙️ Ghi âm" → `audioRecorder.tsx` (MediaRecorder, tự chọn mime webm-opus/mp4, timer + auto-stop maxSec, nghe lại trước khi dùng, PermissionHelp kind='microphone') → `attachments:create`. Config thời lượng tối đa. Chain AI STT (ai-column) để ra text. | done |
