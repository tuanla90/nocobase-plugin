# PRINT-TEMPLATE-NOTES — kinh nghiệm & kiến trúc @ptdl/plugin-print-template
...
## 1. Kiến trúc 3 lane + @ptdl/shared
...
- `@ptdl/shared` là workspace package **bundle vào dist/node_modules** của plugin. Xuất:
  `escapeHtml`, `formatDate` (subpath `@ptdl/shared/format` — thuần, KHÔNG @formily),
  `FieldPickerCascader`, `getCaretElement`, `insertAtCaret`, `ColorField`, `COLOR_PRESETS`,
  `setIconRegistry`.
- **Bẫy @formily/react khi build**: `@ptdl/shared/dist/index` kéo `@formily/react` → rspack
  báo "Can't resolve". Cách chữa: (a) import util thuần từ subpath `@ptdl/shared/format`;
...
`createRenderer()` + helper: formatNumber/formatDate (từ `@ptdl/shared/format`)/`sql` (alasql lazy)/
...
- Stage src + 6 marker + `package.json` vào build-env; giữ REAL `handlebars`/`qrcode-generator`/
  ...
  `@ptdl/shared` từ packages/ (npm prune mỗi lần install); `nocobase-build --tar --no-dts`.
...
- Deploy nb-local: giải nén tgz vào CẢ `node_modules/@ptdl/...` VÀ `storage/plugins/@ptdl/...`
...
Tách `GrapesEditorCore` vào `@ptdl/shared` trước; đổi preset newsletter + inline CSS (`juice`) +
