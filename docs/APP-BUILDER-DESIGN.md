...
- **Chart title in the filter's "Connect fields" picker.** That dialog reads the ChartBlockModel's `title`, patched by `deriveBlockTitle` in `@ptdl/plugin-block-custom-html`. Its old "from query" branch emitted friendly field titles ("Tổng tiền / Trạng thái"), never the aggregation. Changed to the SAME `agg(measureLeaf) theo dimensionLeaf` formula `listCharts` uses, so the two pickers read verbatim-identical labels. (Ships in block-custom-html, not app-builder.)
- **Split-view launcher.** The dashboard launcher is a right-side panel that shrinks the page instead of overlaying it — it portals into the framework's built-in `#nocobase-embed-container` (a flex sibling of the content; give it a width → the page reflows beside it; reset to `auto` on close), the exact mechanism `@ptdl/plugin-detail-panel` uses (replicated, not imported). Body split into "Create" / "Edit dashboard" Tabs; emoji → Lucide icons (`<Icon type="lucide-*">` guarded by `icons.has`, antd fallback); 32px controls. Gotcha: `lucide-wand-2` mis-registers (kebab drops the digit-dash) → use `lucide-wand-sparkles`.
...
## Files
`packages/@ptdl/plugin-app-builder/` — `src/shared/{appSpec.ts, compiler.tsx, extractor.tsx, materialize.tsx, quickView.tsx, dashboard.tsx, launcherDock.ts}`,
