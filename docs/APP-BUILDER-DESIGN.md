...
> Plugin: **`@tuanla90/plugin-app-builder`** · Target NocoBase 2.1.19 · Trạng thái: **P0 (nền) đang dựng.**
> Mục tiêu cuối: user gõ mô tả tiếng Việt → AI sinh **cả app** (collections + quan hệ + nhiều trang + menu +
> widget @tuanla90). Plugin này là **compiler + bộ mẫu (golden corpus)**; lớp AI gắn sau (P2).
...
3. **Tác giả hoá mẫu KHÔNG viết JSON tay** — dựng app thật bằng chính bộ @tuanla90 → **extract** ra App-Spec →
...
| Vòng sinh→validate→retry (P2) | `@tuanla90/shared` `AiCodegenButton` + `generateCode`/`ai-server` |
...
**Đóng gói:** plugin RIÊNG `@tuanla90/plugin-app-builder` (không nhồi vào instant-create-page — khác scope:
whole-app + collection-gen + AI). Phần builder trang import lại từ instant-create-page hoặc tách chung ra
`@tuanla90/shared` sau (P1 để nguyên, copy builders nếu cần độc lập).
...
interface FieldSpec {
  ...
  unique?: boolean;
  defaultValue?: any;
  widget?: string;                           // NHÃN thân thiện của binding @tuanla90 (mục 4). vd 'Progress bar'
...
`boolean · date · datetime · time · color · icon · json` + **mở rộng @tuanla90:** `statusFlow` (kèm `states[]`).
...
## 4. Từ vựng widget (intent → @tuanla90) — bridge tới 28 plugin
...
| intent / interface | widget @tuanla90 (nhãn) | plugin |
|---|---|---|
...
User chốt tầm nhìn: **app-builder là bộ ĐIỀU PHỐI, ghép các plugin @tuanla90 có sẵn — KHÔNG tự chế lại.**
...
wire qua @tuanla90/plugin-formula. *(Trước đây để P4 — nay nâng lên theo yêu cầu user. Validate đã có: computed
...
**P0 — Nền**
- [x] Scaffold `@tuanla90/plugin-app-builder` (package.json + entries client-v2/client/server/locale + recipe). *(2026-07-17)*
...
- [x] Deploy nb-local (recipe + add-markers + node_modules/@tuanla90 + INSERT applicationPlugins id 113 + pm2 restart);
...
- [x] **status-flow** ✅ **XONG + VERIFY LIVE.** `fieldDef('statusFlow')` sinh field THẬT: `type:'string'`,
  ...
  **Fix THẬT (đã áp + verify):** hook `statusFlowFields` lọc field theo `f.options.interface` = undefined runtime cho
  ...
  Sửa **`@tuanla90/plugin-status-flow`** (server hook), build+deploy nb-local. Seed KHÔNG set status (hook lock record mới về
  initial 'Nháp').
...
- **Server** `appBuilder:aiGenerate({description})` → `getAiProvider(app)` (`app.pm.get('ai').aiManager` →
  ...
  app-builder tự chứa, không dep `@tuanla90/shared`) → NL → App-Spec (chuỗi JSON) + **validate/retry ≤3** (nhét lỗi
...
- Component = **`PtdlRichSelectFieldModel`** (@tuanla90 field-enhancements "Rich select"; fallback core `RecordSelectFieldModel`
...
3. **Menu = sidebar groups cho ALL, ít item top-menu** (user: "apply thư viện quản lý sidebar", "hạn chế phân trang menu chính").
   ...
   (`@tuanla90/plugin-menu-enhancements`: route `options.ptdlMenuKind='divider'` + `ptdlMenuStyle`, ghi thẳng lúc tạo route —
   ...
   **FALLBACK (v0.3.2):** detect `app.pm.get('@tuanla90/plugin-menu-enhancements')` — CÓ plugin → divider (trên); KHÔNG có →
...
- **Page-builder refactor = option B (sync guard).** Both page-builders are deliberately shared-free; instead of merging into @tuanla90/shared, `build-env/checks/quickview-sync.mjs` fails the build if the 10 byte-identical helpers diverge between app-builder/quickView.tsx and instant-create-page/quickView.tsx, or if any of 10 feature markers is missing from either. Wired into both recipes.
...
- **Refine an EXISTING chart (v0.5.5–0.5.6).** aiRefineChart only reachable right after generating was too narrow, so `appBuilder:listCharts` scans `flowModels` for every `ChartBlockModel` (SQL `LIKE '%ChartBlockModel%'`, parse options → `{uid, title (from raw's `title.text` or measure/dim), chartType, collection}`) and the 📊 Dashboard modal grows a "✏️ Edit an existing chart with AI" picker: load charts → pick one → describe → aiRefineChart. Works on ANY chart, including hand-built ones (converts a `mode:basic` chart to custom on first refine). v0.5.6 **scopes to the current page by default** (`{pageSchemaUid}` + a This-page/Whole-app toggle) — a whole-app list is cluttered by orphaned charts from deleted dashboards. The tree has no parent column; each model stores `options.parentId`, so listCharts builds a cheap uid→parentId map with SQLite `json_extract(options,'$.parentId')` and walks chart → grid → tab → page uid.
...
## 23. Dashboard polish — filter fan-out, HTML-card edit, tabbed split-view (v0.6.0, 2026-07-19)

Five parallel investigations (one agent each), integrated into one build.

- **Filter → charts was dead.** A `FilterFormItemModel`'s `defaultTargetUid` is NOT the live wiring — it's a one-time seed the interactive "Add filter field" UI (`FilterFormGridModel.onModelCreated`) consumes to populate ONE entry in the page's FilterManager. The REAL, plural connection is a `filterManager: [{ filterId, targetId, filterPaths }]` array stored on the shared **`BlockGridModel`** (read by `BlockGridModel.onInit`; re-serialized on save). `FilterFormItemModel.doFilter()` → `filterManager.refreshTargetsByFilter(this.uid)` only reads entries whose `filterId === the item's own uid`. `createModelAsync` never runs `onModelCreated`, so a raw-built filter looked wired but did nothing. Fix: `createDashboard` builds the array itself — one entry per (filter field × every chart AND KPI card) — and threads it onto the grid literal as a top-level sibling of `subModels`. `filterId` = each FilterFormItemModel's own uid; `filterPaths` is an array; a relation filter field files on the target key (`khach_hang.id`, via `filterTargetKey`), not the bare relation name.
- **Edit HTML cards, not just charts.** `CustomHtmlBlockModel extends ChartBlockModel` and shares the same `configure.query` path; it only diverges in where the editable code lives (`customHtmlSettings.code.code`, an HTML-returning JS string seeing `data`/`helpers`, vs the chart's `chart.option.raw` seeing `ctx`). `listCharts` now matches `%ChartBlockModel% OR %CustomHtmlBlockModel%` and labels HTML cards `chartType:'html'`; `aiRefineChart` reads `options.use` from the loaded record and branches (chart → rewrite `option.raw` with `chartRefineSystemPrompt`; html → rewrite `customHtmlSettings.code.code` with a new `htmlRefineSystemPrompt`). No client change (the picker's label is a plain template string).
- **Chart title in the filter's "Connect fields" picker.** That dialog reads the ChartBlockModel's `title`, patched by `deriveBlockTitle` in `@tuanla90/plugin-block-custom-html`. Its old "from query" branch emitted friendly field titles ("Tổng tiền / Trạng thái"), never the aggregation. Changed to the SAME `agg(measureLeaf) theo dimensionLeaf` formula `listCharts` uses, so the two pickers read verbatim-identical labels. (Ships in block-custom-html, not app-builder.)
- **Split-view launcher.** The dashboard launcher is a right-side panel that shrinks the page instead of overlaying it — it portals into the framework's built-in `#nocobase-embed-container` (a flex sibling of the content; give it a width → the page reflows beside it; reset to `auto` on close), the exact mechanism `@tuanla90/plugin-detail-panel` uses (replicated, not imported). Body split into "Create" / "Edit dashboard" Tabs; emoji → Lucide icons (`<Icon type="lucide-*">` guarded by `icons.has`, antd fallback); 32px controls. Gotcha: `lucide-wand-2` mis-registers (kebab drops the digit-dash) → use `lucide-wand-sparkles`.

## 24. Launcher dock + add-widget + hover chart-edit + 3-tab Build-app modal (v0.6.1 → v0.6.9, 2026-07-19)

Iterations on the two floating launchers (Build app + 📊 Dashboard) and the Build-app modal.

- **Deduped ERD + Delete buttons (v0.6.1–0.6.2).** SpecPreview's ERD collapses a m2o + its reverse o2m into one bidirectional arrow (cardinality 1–1/1–N/N–N) and orders boxes by relation adjacency (BFS) so an unrelated box can't sit between two linked ones and read as a false link. Every generated table/detail view gains a row/record `DeleteActionModel` (both page-builders, `quickView.tsx`, byte-identical).
- **Shared draggable dock (v0.6.3–0.6.4).** The per-plugin fixed-position buttons became one `#ptdl-launcher-dock` (`src/shared/launcherDock.ts`, byte-identical in instant-create-page, sync-guarded): a draggable + collapsible vanilla-DOM container every page-builder portals its buttons into (`ensureLauncherDock()` → `.ptdl-dock-items`, `alignItems:stretch` → equal width). Handle colour is copied from the first rendered `.ant-btn-primary` background via a MutationObserver (NocoBase's theme leaves `--ant-color-primary` blank). Position + collapsed state persist in localStorage. The generated-page result link uses client-side `app.router.navigate` (not `<a href>`) so opening it doesn't F5 away the just-generated chat result.
- **AI add-a-widget to an existing dashboard (v0.6.5).** `aiAddWidget({pageSchemaUid, description})` finds the `BlockGridModel` under a page (sqlite `json_extract` ancestry walk), reads its collection + existing chart/KPI uids + max sort, and AI-designs one widget; the client `addWidgetToDashboard` materializes it (`createModelAsync` under the grid) then `appBuilder:patchGrid` appends the layout row + merges any new `filterManager` entries.
- **Hover Edit-chart-with-AI overlay (v0.6.7–0.6.8).** Every Chart / Custom-HTML block on a page shows a ✨ button on hover (edit-mode only, anchored on the full-block `data-float-menu-model-uid`) → type an instruction → `aiRefineChart` rewrites the block's ECharts / HTML code → the block re-renders IN PLACE via `refreshBlockLive` (`setStepParams` + `applyFlow`, no page reload); one-step Undo restores the prior code via `setChartRaw`.
- **3-tab Build-app modal (v0.6.9).** The "Tả là dựng (AI)" modal — previously one long stacked column (describe → Generate/Step-by-step → Preview/JSON segmented → separate refine input → actions) — is now three tabs:
  - **New module** — one description textarea → **[Create new spec]** (`aiGenerate`; a fresh app that REPLACES the editor, with a confirm guard) / **[Edit spec]** (`aiRefine`; applies the text to the current spec). Both drive the SAME textarea. Live `SpecPreview` + the shared Create-app / Delete / result below.
  - **Modify running app** — its own textarea → **[Plan the change]** (`aiPlan`) → review the tool-call steps → Run; for editing an ALREADY-built app (its input is separate `planDesc` state so it doesn't fight tab 1).
  - **JSON** — the raw App-Spec editor + Load demo + Validate.
  Create-app / Delete / created-pages result are a shared `renderCreateActions()` fragment across the two spec tabs. Drops the old Preview/JSON `Segmented` toggle + the standalone refine `Input` (`specView` / `refineText` state removed). All buttons Lucide-iconed, medium size. Verified server-side (served `dist/client-v2/index.js` byte-matches the deployed tgz + carries the new tab strings) since the local preview is stuck on stale auth — a browser reload of `/v/` shows it.

## 25. Dashboard-panel fixes — icon align, refinable KPIs, filter labels, manual filter column (v0.6.10, 2026-07-19)

Four polish requests on the AI-Dashboard side panel (plus a hover-overlay button-position tweak, done separately).

- **Icon vertical alignment.** The shared `LIcon` rendered NocoBase's `<Icon type="lucide-*">` as an inline SVG whose baseline descender gap pushed the glyph visually ABOVE the text centre. Now it renders `display:inline-flex; align-items:center` (no inline gap) + `vertical-align:-0.125em` (matching how antd's own icons sit inline) + `line-height:1`. One change fixes every icon in the plugin.
- **Refinable HTML KPI cards.** `createDashboard` now returns the score-card uids too (`chartType:'html'`), not just ECharts charts, so the launcher's "Edit a chart with AI" list includes them; the refine-list icon switches to `lucide-hash` for html. `aiRefineChart` already branches on the block's `use` (ChartBlockModel → `option.raw`; CustomHtmlBlockModel → `customHtmlSettings.code.code`), so the KPI card's HTML is rewritten in place.
- **Filter display-name override.** `filterBlock` gained an optional `labels: {field→name}` map; the built `FilterFormItemModel.filterFormItemSettings.init.filterField.title` prefers it over the auto-resolved `uiSchema.title`. This matters because some fields (esp. relation FK columns like `khach_hang_id`) resolve to their raw name.
- **Manual filter column.** New read-only server action `appBuilder:dashboardFields({pageSchemaUid})` — the same grid/target/collection walk `aiAddWidget` does (sqlite `json_extract` ancestry → `BlockGridModel` → its child chart/KPI uids + collection + maxSort), PLUS the collection's filterable field list (excludes to-many/password; strips `{{t('…')}}` titles). The Edit tab uses it for a manual picker: **Load fields → multi-select field(s) + optional per-field display name → Add filter** → builds a `{kind:'filter', fields, filterLabels}` widget and inserts it via the proven `addWidgetToDashboard` (one filter bar wired to every chart + KPI). Verified live: `dashboardFields` returns `{gridUid, collection, chartUids, scoreUids, nextSort, fields}` for a real dashboard (`don_hang`: 4 charts, 3 scores, 14 fields).

## Files
`packages/@tuanla90/plugin-app-builder/` — `src/shared/{appSpec.ts, compiler.tsx, extractor.tsx, materialize.tsx, quickView.tsx, dashboard.tsx, launcherDock.ts}`,
