...
5. **MỚI `shared/relativeTime`** — gom `block-custom-html timeAgo` (`render.ts:228`), `change-log relativeTime` (`changeLogClient.ts:113`), `field-enh` reduceUnit/todayLabel. **Rủi ro trung bình:** 3 bản khác nhau (vi-only vs bilingual `5m ago` vs i18n smart-units) → API `relativeTime(date,{locale,style,t})`.
6. **MỚI `shared/aggregate`** — `sum/avg/min/max/count/groupBy/median/range` một null-policy; seed từ `buildHelpers`. 5 plugin dùng lại. Rủi ro thấp.
