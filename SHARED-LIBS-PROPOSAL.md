# @tuanla90 — đề xuất chuẩn hoá thư viện dùng chung
...
> @tuanla90 plugins … packages build independently, so no cross-package import"*). Hệ quả: cùng một widget
...
- **0 import `@tuanla90/*` → `@tuanla90/*`.** Đúng với mô hình bundle-độc-lập của NocoBase (import thật sẽ nhét
...
## 3. Kiến trúc đề xuất: `packages/@tuanla90/shared/` (source lib được BUNDLE, không phải runtime dep)
...
**✅ Spike ĐÃ XONG (2026-07-11):** tạo `packages/@tuanla90/shared` (pre-compiled dist), đặt vào
`build-env/node_modules/@tuanla90/shared`, cho `plugin-menu-badge` `import { colorToString } from '@tuanla90/shared'`,
build → marker của shared **xuất hiện trong `dist/client-v2/index.js`** và **không còn import `@tuanla90/shared`
ngoài** ⇒ rspack đã **inline** (bundle) như lucide. Cơ chế chốt = **(a) node_modules-resolvable package**.
`@tuanla90/shared` không nằm mkstub-list nên không bị externalize. (menu-badge đã revert về nguyên trạng.)
...
- **P1 — Color kit + Icon helper**: tạo `@tuanla90/shared`, spike bundling, migrate màu + icon trước (ROI cao
...
- **Khối runtime "Section" trong form/detail** (bấm thu/xoè, giống sidebar) → **CHỐT: gộp vào block-tabs, đổi tên
  `@tuanla90/plugin-layout-containers`** ("Layout containers"): cùng một *container-core chứa grid con*; Tabs / Collapse /
...
- **Cơ chế chia sẻ = bundle** `@tuanla90/shared` (source lib compile thẳng vào từng plugin; **không** kit-plugin).
  ⇒ mỗi plugin deploy **độc lập**, zero runtime dep vào plugin @tuanla90 khác. Đã **spike xác nhận** (mục 3).
...
- **Gộp collapse → `@tuanla90/plugin-layout-containers`** (mục 5).

## 8. Cơ chế build & propagation ("sửa A thì B,C,D build lại")
- `@tuanla90/shared` khai báo là **`devDependency`** của mỗi plugin (bundle lúc build, KHÔNG phải runtime dep)
...
- [x] **Lát cắt thật (menu-badge)**: `COLOR_PRESETS`+`colorToString` → `@tuanla90/shared`; build (`recipes/run-shared-build.sh`)
  + build menu-badge + deploy localhost. Verify served client-v2 đã inline, không import ngoài. **Finding:**
  client lane **inline** shared; server lane (tsup) **vendored** bản sao vào `dist/node_modules/@tuanla90/shared` —
  cả hai self-contained (plugin chạy độc lập). devDep `@tuanla90/shared` đã thêm vào menu-badge (cho lerna graph).
...
- [x] **P1a — `COLOR_PRESETS` + `colorToString`** → `@tuanla90/shared` cho **8 plugin** (menu-badge + block-tabs,
...
- [x] **P4a field-picker lazy** → `@tuanla90/shared/fieldPicker.tsx` (bản ai-column). Migrate ai-column, global-search,
...
- [x] **P4b format helpers** → `@tuanla90/shared/format` (number superset data-viz, date superset print-template,
  ...
  import kéo antd cho consumer thuần → thêm **subpath `@tuanla90/shared/format`** (pure, không antd) cho data-viz.
- **→ "MIGRATE TẤT CẢ" (P1–P4) HOÀN TẤT.** Còn lại là phase SAU roadmap ↓
- [x] **settings-kit** → `@tuanla90/shared/settingsKit.tsx`: `SettingsGrid`, `fieldItem`/`fi`, `rx`/`visibleWhen`,
...
- [x] **tooling `check-stale`** (`recipes/check-stale.sh`) — báo plugin nào có tgz cũ hơn `@tuanla90/shared/dist` → cần build lại.
...
- [x] **Đổi tên `block-tabs` → `@tuanla90/plugin-layout-containers` (v0.3.0)** — mv source dir + package.json + recipe mới
...
- [x] **Gộp `menu-sections` + `menu-badge` → `@tuanla90/plugin-menu-enhancements`** — cả 2 client-only, config trên route
...
- **→ TOÀN BỘ ROADMAP HOÀN TẤT.** `@tuanla90/shared` + `loginKit` + `condition`; check-stale sạch; docs/memory cập nhật cho session sau.
...
**d) Relation-appends cascader — ✅ GOM XONG (2026-07-15).** Audit §10 gốc bỏ sót 1 dup: cascader "chọn quan hệ nhiều cấp, hover xổ con, mỗi pick thành tag" (build options từ `collections:get?appends=fields`, depth 3) copy-paste ở print-template (`AppendsPicker`) và line-generator (`RelationAppendsPicker`). Đã extract → **`@tuanla90/shared` `RelationAppendsPicker` + `buildRelationOptions`** (`relationPicker.tsx`, dùng chung cache `getFields` của field-picker, i18n `st()`, hint = prop); 2 plugin refactor sang bản shared, line-generator được wire thêm shared-i18n (trước đó thiếu). Build + deploy + verify served bundle. Chi tiết: `docs/SHARED-DEDUP-AUDIT.md §3b`.
