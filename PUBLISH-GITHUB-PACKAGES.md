...
Không cần build lại gì cả: **30 file `.tgz` đã sẵn** trong `latest/@ptdl/`. Chỉ cần đẩy chúng lên.
...
## Vì sao publish dưới tên `@tuanla90/*` (không phải `@ptdl/*`)

GitHub Packages bắt buộc **scope npm = tên chủ GitHub**. `@ptdl` đã bị một tổ chức
...
`name == thư mục == @ptdl/plugin-X`), ta giữ nguyên workspace là `@ptdl` và **chỉ đổi
...
> Dev (nb-local) vẫn chạy `@ptdl/plugin-X`; server production chạy `@tuanla90/plugin-X`.
...
# publish TẤT CẢ tgz trong latest/@ptdl:
node build-env/publish-ghp.cjs
```
...
Script làm gì: giải nén từng `.tgz` → đổi `name` `@ptdl/…`→`@tuanla90/…` + thêm
...
## Phần 5 — Server đang chạy `@ptdl/*` (migrate)

Server nào trước đây cài `@ptdl/plugin-X` bằng tay (giải nén tgz vào `node_modules/@ptdl`)
...
2. Gỡ bản `@ptdl` cũ (`pm disable` rồi `pm remove`) để tránh chạy 2 bản.
...
## (Tùy chọn tương lai) Đổi tên tận gốc `@ptdl → @tuanla90`

Nếu sau này muốn dev == prod cùng tên: đổi `name` trong 31 `package.json`, rồi sửa mỗi
`build-env/recipes/run-*.sh` cho `PKG="@tuanla90/plugin-X"` **nhưng giữ** `SRC=".../packages/@ptdl/plugin-X"`
(thư mục workspace) và các tham chiếu `@ptdl/shared`. Khi đó bỏ được bước rewrite của
