...
**Group:** UI · Action buttons (Actions/UI) · **Runs on:** /admin (classic) + /v/ (modern) · **Version:** 0.1.2
...
Vietnamese), reuses the `@tuanla90/shared` colour kit.

**Build:**

```bash
cd build-env && bash recipes/run-action-enh-build.sh
bash recipes/add-markers.sh storage/tar/@tuanla90/plugin-action-enhancements-*.tgz
```
