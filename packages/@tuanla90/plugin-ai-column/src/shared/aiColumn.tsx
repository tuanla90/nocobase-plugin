...
/**
 * @tuanla90/plugin-ai-column — shared editable field model.
 *
      ...
      properties: {
        ...
        tabModel: {
          ...
          properties: {
            ...
            aiHint: { type: 'string', title: t('Chú thích nút ✨ (hiện khi hover)'), 'x-decorator': 'FormItem', 'x-decorator-props': tight, 'x-component': 'PtdlHintInput' },
          },
        },
      },
    },
...
/** Register flow-settings components ONLY if not already registered — the registry is GLOBAL and
 *  shared across every AI field/action, so registering a shared name (FormTab, voice/image pickers…)
 *  from multiple files spams "Component X is already registered and will be overwritten". This skips
 *  names already present so each shared component is registered exactly once. */
export function registerFlowComponentsOnce(flowEngine: any, comps: Record<string, any>) {
  try {
    const fs = flowEngine?.flowSettings;
...
/** Optional hover-tooltip override for the ✨ button — lets the designer label what THIS button does
 *  (e.g. "Tạo mô tả sản phẩm"). Reused by EVERY AI field component (the flow-settings component registry
 *  is global, so registering it once here makes 'PtdlHintInput' resolvable in all their uiSchemas). */
export const PtdlHintInput: React.FC<any> = observer((props: any) => (
  <Input value={props.value} placeholder={t('vd: Tạo mô tả sản phẩm — để trống = mặc định')} onChange={(e) => props.onChange?.(e.target.value)} allowClear />
));
...
export function registerAiColumn({ flowEngine, variants, EditableItemModel, api, tExpr }: Deps) {
  ...
  try {
    registerFlowComponentsOnce(flowEngine, AI_SETTINGS_COMPONENTS);
  } catch (e) {
    // eslint-disable-next-line no-console
