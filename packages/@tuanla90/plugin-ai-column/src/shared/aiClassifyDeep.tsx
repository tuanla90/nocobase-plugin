...
/**
 * @tuanla90/plugin-ai-column — "AI Phân loại chuyên sâu" (deep/decision-support classify). For HARD
...
/** UNIFIED aspects list — ONE row per aspect (name + score weight), replacing the duplicated
 *  "attributes to understand" + "rubric criteria" lists (users were typing Vật-liệu/Chức-năng twice).
 *  The AI EXTRACTS every aspect (to understand the input, shown in the result panel) AND SCORES each
 *  aspect with weight>0 (blank/0 = understand-only, not scored). One place, no duplication. */
export const PtdlDeepAspects: React.FC<any> = observer((props: any) => {
  const rows: any[] = Array.isArray(props.value) ? props.value : [];
  const update = (i: number, patch: any) => { const n = rows.slice(); n[i] = { ...n[i], ...patch }; props.onChange?.(n); };
  const add = () => props.onChange?.([...rows, { name: '', weight: 20 }]);
  const rm = (i: number) => props.onChange?.(rows.filter((_: any, idx: number) => idx !== i));
  const total = rows.reduce((s, r) => s + (Number(r.weight) || 0), 0);
  const { token } = theme.useToken();
  const tv = themeVars(token);
  return (
    <div style={tv}>
      {rows.length ? (
        <div style={{ display: 'flex', gap: 8, marginBottom: 4, fontSize: 12, color: 'var(--colorTextTertiary)' }}>
          <span style={{ flex: 1 }}>{t('Khía cạnh (AI vừa hiểu vừa chấm)')}</span>
          <span style={{ width: 90 }}>{t('Điểm chấm')}</span>
          <span style={{ width: 24 }} />
        </div>
      ) : null}
      {rows.map((r, i) => (
        <div key={i} style={{ display: 'flex', gap: 8, marginBottom: 6, alignItems: 'center' }}>
          <span style={{ flex: '0 0 auto', width: 18, height: 18, borderRadius: 9, background: '#f0e9fb', color: '#7c3aed', fontSize: 11, fontWeight: 700, display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>{i + 1}</span>
          <Input style={{ flex: 1 }} placeholder={t('vd: Vật liệu, Chức năng, Cách đóng gói')} value={r.name} onChange={(e) => update(i, { name: e.target.value })} />
          <Input type="number" style={{ width: 90 }} placeholder={t('0 = chỉ hiểu')} value={r.weight} onChange={(e) => update(i, { weight: Number(e.target.value) || 0 })} />
          <Button danger type="text" style={{ width: 24 }} onClick={() => rm(i)}>✕</Button>
        </div>
      ))}
      <Button type="dashed" onClick={add} style={{ width: '100%' }}>{t('+ Thêm khía cạnh')}</Button>
      {rows.length ? <div style={{ fontSize: 12, color: total === 100 ? '#52c41a' : '#d48806', marginTop: 4 }}>{t('Tổng điểm chấm')}: {total}{total !== 100 ? t(' (nên = 100)') : ' ✓'}</div> : <div style={{ fontSize: 12, color: 'var(--colorTextTertiary)', marginTop: 4 }}>{t('Mỗi khía cạnh: AI trích để HIỂU input + CHẤM ứng viên theo điểm. Để điểm = 0 nếu chỉ muốn hiểu, không chấm. Bỏ trống hết = AI tự chấm tổng quát.')}</div>}
    </div>
  );
});
...
export const PtdlDeepExamples: React.FC<any> = observer((props: any) => {
  ...
  return (
    <div style={tv}>
      ...
      {v.collection ? (() => {
        // Filter which example rows count (e.g. is_verified = true) — supports using the SAME fact table
        // as its own verified-examples source. Value = v.filter {conditions, match}. Tokens: {{col}}.
        const fv = v.filter || {};
        const conds: any[] = Array.isArray(fv.conditions) ? fv.conditions : [];
        const setF = (conditions: any[], match?: string) => set({ filter: { match: match ?? fv.match ?? 'all', conditions } });
        return (
          <div style={{ marginTop: 6 }}>
            {conds.length > 1 ? (
              <Select style={{ width: 200, marginBottom: 6 }} value={fv.match || 'all'} onChange={(m) => setF(conds, m)}
                options={[{ value: 'all', label: t('Khớp TẤT CẢ điều kiện') }, { value: 'any', label: t('Khớp BẤT KỲ điều kiện') }]} />
            ) : null}
            {conds.map((c: any, i: number) => (
              <div key={i} style={{ marginBottom: 6 }}>
                <ConditionRow api={API} collectionName={v.collection} dataSourceKey={v.dataSourceKey || 'main'}
                  path={c.field ? String(c.field).split('.') : []} op={c.op} value={c.value}
                  onChange={(next: any) => { const n = conds.slice(); n[i] = { field: (next.path || []).join('.'), op: next.op, value: next.value }; setF(n); }}
                  onRemove={() => setF(conds.filter((_: any, idx: number) => idx !== i))} />
              </div>
            ))}
            <Button type="dashed" onClick={() => setF([...conds, { field: '', op: '$eq', value: undefined }])} style={{ width: '100%' }}>
              {t('+ Lọc ví dụ (vd: đã xác thực = true)')}
            </Button>
          </div>
        );
      })() : null}
      ...
      properties: {
        tabMatch: {
          ...
          properties: {
            ...
            aiFilter: { type: 'object', title: t('Lọc phạm vi master (tùy chọn)'), 'x-decorator': 'FormItem', 'x-decorator-props': { tooltip: t('Chỉ đối chiếu trong các dòng master khớp điều kiện → nhanh + chính xác hơn (giống data-scope).') }, 'x-component': 'PtdlMasterFilter' },
        ...
        tabDeep: {
          ...
          properties: {
            ...
            aiAspects: { type: 'array', title: t('① Khía cạnh AI vừa HIỂU vừa CHẤM (tên + điểm)'), 'x-decorator': 'FormItem', 'x-decorator-props': { tooltip: t('Mỗi khía cạnh (vd Vật liệu, Chức năng) khai MỘT LẦN: AI vừa trích để hiểu input (hiện ở panel “AI đã hiểu…”) vừa chấm điểm ứng viên theo trọng số. Điểm = 0 → chỉ hiểu, không chấm. Bỏ trống hết → AI tự chấm tổng quát.') }, 'x-component': 'PtdlDeepAspects' },
  ...
  return {
    ...
    steps: {
      ai: {
        ...
        defaultParams: { aiService: '', aiModel: '', aiMaster: {}, aiQueryFields: [], aiWriteField: '', aiDisplayFields: [], aiTopK: 20, aiRoleHint: '', aiAspects: [], aiAttributes: [], aiRubric: '', aiRubricItems: [], aiExamples: {}, aiFeedback: true, aiHint: '', aiFilter: {} },
        handler(ctx: any, params: any) {
          ...
          ctx.model.setProps('aiAspects', Array.isArray(params?.aiAspects) ? params.aiAspects : []);
          ctx.model.setProps('aiExamples', params?.aiExamples || {});
          ctx.model.setProps('aiFilter', params?.aiFilter || {});
...
export const AiClassifyDeepEditable: React.FC<{ model: any; baseRender: () => React.ReactNode; relationMode?: boolean }> = observer(({ model, baseRender, relationMode }) => {
  ...
  const onGen = async () => {
    ...
    try {
      ...
      // UNIFIED aspects → derive both attributes (extract/understand) + rubricItems (score) from ONE list.
      // Back-compat: fall back to the old separate aiAttributes/aiRubricItems if no aspects configured.
      const aspects = (Array.isArray(p.aiAspects) ? p.aiAspects : []).filter((a: any) => a?.name && String(a.name).trim());
      const attributes = aspects.length
        ? aspects.map((a: any, i: number) => ({ name: slugKey(a.name, i), description: a.name }))
        : (Array.isArray(p.aiAttributes) ? p.aiAttributes.filter((a: any) => a?.name) : []);
      const rubricItems = aspects.length
        ? aspects.filter((a: any) => Number(a.weight) > 0).map((a: any) => ({ criterion: a.name, weight: Number(a.weight) || 0 }))
        : (Array.isArray(p.aiRubricItems) ? p.aiRubricItems.filter((r: any) => r?.criterion) : []);
      const res = await API.request({
        ...
        data: {
          ...
          rubricItems: rubricItems.length ? rubricItems : undefined,
          ...
          examples: p.aiExamples?.collection && p.aiExamples?.queryField && p.aiExamples?.codeField
            ? { collection: p.aiExamples.collection, dataSourceKey: p.aiExamples.dataSourceKey || 'main', queryField: p.aiExamples.queryField, codeField: p.aiExamples.codeField, masterCodeField: p.aiWriteField || undefined, k: p.aiExamples.k || 3, filter: resolveFilterPayload(p.aiExamples.filter, values) }
            : undefined,
          filter: resolveFilterPayload(p.aiFilter, values),
      ...
      attributes.forEach((a: any) => { if (a?.name) attrLabels[a.name] = a.description || a.name; });
  ...
  return (
    <div style={{ display: 'flex', gap: 4, width: '100%', alignItems: 'flex-start' }}>
      ...
      <Tooltip title={!canGen ? t('Chưa cấu hình: chọn bảng master + nội dung đối chiếu (mở field settings)') : (p.aiHint || t('Phân loại chuyên sâu — AI chấm & giải trình từng ứng viên'))}>
...
export function registerAiClassifyDeep({ flowEngine, variants, EditableItemModel, api, tExpr }: Deps) {
  ...
  try {
    registerFlowComponentsOnce(flowEngine, { PtdlDeepAttributes, PtdlDeepRubric, PtdlDeepRubricRows, PtdlDeepAspects, PtdlDeepDisplayFields, PtdlDeepExamples, PtdlDeepRoleHint, PtdlDeepFeedback, FormTab, 'FormTab.TabPane': FormTab.TabPane });
  } catch (e) {
