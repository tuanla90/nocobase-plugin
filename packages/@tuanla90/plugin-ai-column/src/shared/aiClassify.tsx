...
/**
 * @tuanla90/plugin-ai-column — "AI Phân loại" (Block B, surface 2): match this field's value to ONE row
...
/** Build the classify `filter` payload from the aiFilter config, resolving `{{col}}` tokens in each
 *  condition value against the CURRENT record (dynamic data-scope). Returns undefined when empty. */
export function resolveFilterPayload(aiFilter: any, values: Record<string, any>): any {
  const conds = Array.isArray(aiFilter?.conditions) ? aiFilter.conditions.filter((c: any) => c?.field && c?.op) : [];
  if (!conds.length) return undefined;
  const resolveTok = (val: any) =>
    typeof val === 'string'
      ? val.replace(/\{\{\s*([\w.$-]+)\s*\}\}/g, (_m: string, path: string) => {
          const v = String(path).split('.').reduce((a: any, k: string) => (a == null ? undefined : a[k]), values);
          return v == null ? '' : String(v);
        })
      : val;
  return { match: aiFilter?.match || 'all', conditions: conds.map((c: any) => ({ field: c.field, op: c.op, value: resolveTok(c.value) })) };
}

/** SCOPE filter (data-scope) on the MASTER — restrict which master rows are compared BEFORE the vector
 *  cosine (fewer vectors = faster + no cross-group false matches). Rows use the shared ConditionRow
 *  scoped to `aiMaster.collection`. Value = {match:'all'|'any', conditions:[{field,op,value}]}. A
 *  condition value may reference the current record via `{{colName}}` (resolved at run time). */
export const PtdlMasterFilter: React.FC<any> = observer((props: any) => {
  const form = useForm();
  const master = form?.values?.aiMaster || form?.values?.aiClfMaster || {};
  const v = props.value || {};
  const conds: any[] = Array.isArray(v.conditions) ? v.conditions : [];
  const commit = (conditions: any[], match?: string) => props.onChange?.({ match: match ?? v.match ?? 'all', conditions });
  if (!master.collection) return <div style={{ fontSize: 12, color: '#999' }}>{t('Chọn bảng master trước')}</div>;
  return (
    <div>
      {conds.length > 1 ? (
        <div style={{ marginBottom: 6 }}>
          <Select style={{ width: 200 }} value={v.match || 'all'} onChange={(m) => commit(conds, m)}
      ...
      {conds.map((c, i) => (
        <div key={i} style={{ marginBottom: 6 }}>
          <ConditionRow
            ...
            collectionName={master.collection}
            dataSourceKey={master.dataSourceKey || 'main'}
            path={c.field ? String(c.field).split('.') : []}
            op={c.op}
            value={c.value}
            onChange={(next: any) => { const n = conds.slice(); n[i] = { field: (next.path || []).join('.'), op: next.op, value: next.value }; commit(n); }}
            onRemove={() => commit(conds.filter((_: any, idx: number) => idx !== i))}
          />
        </div>
      ))}
      <Button type="dashed" onClick={() => commit([...conds, { field: '', op: '$eq', value: undefined }])} style={{ width: '100%' }}>
        {t('+ Thêm điều kiện lọc phạm vi')}
      </Button>
      <div style={{ fontSize: 12, color: '#888', marginTop: 4 }}>
        {t('Chỉ đối chiếu trong phạm vi lọc → nhanh + chính xác hơn. Giá trị có thể dùng {{cột_của_bản_ghi}} để lọc động.')}
      </div>
    </div>
  );
});
  ...
  return {
    ...
    steps: {
      ai: {
        ...
        defaultParams: { aiService: '', aiModel: '', aiMaster: {}, aiQueryFields: [], aiWriteField: '', aiLabelFields: [], aiTopK: 8, aiAuto: { enabled: false, threshold: 80 }, aiRerank: true, aiHint: '', aiFilter: {} },
...
export const AiClassifyEditable: React.FC<{ model: any; baseRender: () => React.ReactNode }> = observer(({ model, baseRender }) => {
  ...
  return (
    <div style={{ display: 'flex', gap: 4, width: '100%', alignItems: 'flex-start' }}>
      ...
      <Tooltip title={!canGen ? t('Chưa cấu hình: chọn bảng master + nội dung đối chiếu (mở field settings)') : (p.aiHint || t('Phân loại/đối chiếu bằng AI rồi chọn đáp án'))}>
        <Button
