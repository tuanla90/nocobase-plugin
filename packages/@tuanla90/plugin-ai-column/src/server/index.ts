/**
 * @tuanla90/plugin-ai-column — server lane.
 *
...
/** Convert @tuanla90 condition rows [{field,op,value}] → a NocoBase filter object. Used to SCOPE the master
 *  candidate set BEFORE vector/keyword comparison (fewer vectors = faster + fewer wrong-group matches,
 *  like a data-scope). The condition-kit op names ($eq/$ne/$gt/$in/$includes/$empty…) map ~1:1 to
 *  NocoBase filter operators. Returns null when there are no usable conditions. */
function conditionsToFilter(conds: any[], match?: string): any {
  const items = (Array.isArray(conds) ? conds : [])
    .filter((c) => c && c.field && c.op)
    .map((c) => {
      const op = String(c.op);
      const field = String(c.field);
      if (op === '$empty' || op === '$notEmpty') return { [field]: { [op]: true } };
      if (op === '$in' || op === '$notIn') return { [field]: { [op]: (Array.isArray(c.value) ? c.value : String(c.value ?? '').split(',')).map((s: any) => String(s).trim()).filter(Boolean) } };
      return { [field]: { [op]: c.value } };
    });
  if (!items.length) return null;
  return match === 'any' ? { $or: items } : { $and: items };
}
...
export class PluginAiColumnServer extends Plugin {
  ...
  async runClassify(params: {
    ...
    extraPrompt?: string;
    filter?: { conditions: any[]; match?: string };
  }): Promise<any> {
    const db = this.db;
    const dsk = params.dataSourceKey || 'main';
    ...
    const cacheRepo = db.getRepository('ptdlClassifyEmbed');
    let cached = await cacheRepo.find({ filter: { collectionName: params.masterCollection, dataSourceKey: dsk } });
    // Optional SCOPE filter (data-scope): restrict candidates to master rows matching a condition first.
    const scopeFilter = conditionsToFilter(params.filter?.conditions, params.filter?.match);
    if (scopeFilter) {
      try {
        const mc: any = db.getCollection(params.masterCollection);
        const mpk = mc?.model?.primaryKeyAttribute || 'id';
        const arows = await db.getRepository(params.masterCollection).find({ filter: scopeFilter, fields: [mpk], limit: 50000 });
        const allow = new Set((arows || []).map((r: any) => String((r.toJSON ? r.toJSON() : r)[mpk])));
        cached = cached.filter((c: any) => allow.has(String((c.toJSON ? c.toJSON() : c).recordId)));
      } catch (e: any) { this.app.log?.warn?.('[ptdl-ai-column] classify scope filter failed → ignored: ' + (e?.message || e)); }
    }
    ...
    if (!scored.length) {
      ...
      const mrows = await db.getRepository(params.masterCollection).find({ ...(scopeFilter ? { filter: scopeFilter } : {}), limit: 2000 });
  ...
  async runClassifyDeep(params: {
    ...
    examples?: { collection: string; dataSourceKey?: string; queryField: string; codeField: string; masterCodeField?: string; k?: number; filter?: { conditions: any[]; match?: string } };
    filter?: { conditions: any[]; match?: string };
  }): Promise<any> {
    const db = this.db;
    const dsk = params.dataSourceKey || 'main';
    ...
    // 0) Optional SCOPE filter — restrict the candidate set to master rows matching a condition (like a
    // data-scope) BEFORE comparing vectors: fewer vectors = faster + no cross-group false matches.
    const scopeFilter = conditionsToFilter(params.filter?.conditions, params.filter?.match);
    let scopeTks: Set<string> | null = null;
    if (scopeFilter) {
      try {
        const rows = await db.getRepository(params.masterCollection).find({ filter: scopeFilter, fields: [pk], limit: 50000 });
        scopeTks = new Set((rows || []).map((r: any) => String((r.toJSON ? r.toJSON() : r)[pk])));
      } catch (e: any) { this.app.log?.warn?.('[ptdl-ai-column] deep scope filter failed → ignored: ' + (e?.message || e)); }
    }
    // 1) Shortlist — vector if the master is embedded, else lexical keyword.
    let cached = await db.getRepository('ptdlClassifyEmbed').find({ filter: { collectionName: params.masterCollection, dataSourceKey: dsk } });
    if (scopeTks) cached = cached.filter((c: any) => scopeTks!.has(String((c.toJSON ? c.toJSON() : c).recordId)));
    let scored: Array<{ tk: string; sim: number }> = [];
    let method = 'vector';
    ...
    if (ex?.collection && ex.queryField && ex.codeField) {
      try {
        ...
        if (exRepo) {
          ...
          // Optional examples SCOPE filter (e.g. is_verified = true) — lets the SAME fact table serve as
          // its own verified-examples source. Restrict example rows before ranking.
          const exScope = conditionsToFilter(ex.filter?.conditions, ex.filter?.match);
          let exScopeTks: Set<string> | null = null;
          if (exScope) {
            try {
              const arows = await exRepo.find({ filter: exScope, fields: [exPk], limit: 50000 });
              exScopeTks = new Set((arows || []).map((r: any) => String((r.toJSON ? r.toJSON() : r)[exPk])));
            } catch (e: any) { this.app.log?.warn?.('[ptdl-ai-column] examples scope filter failed → ignored: ' + (e?.message || e)); }
          }
          ...
          let exCached = await db.getRepository('ptdlClassifyEmbed').find({ filter: { collectionName: ex.collection, dataSourceKey: exDsk } });
          if (exScopeTks) exCached = exCached.filter((c: any) => exScopeTks!.has(String((c.toJSON ? c.toJSON() : c).recordId)));
          ...
          if (ranked.length) {
            ...
            const exRows = await exRepo.find({ ...(exScope ? { filter: exScope } : {}), limit: 3000 });
          ...
          if (codeField) {
            for (const f of fewShot) {
              ...
              // Respect the scope filter: a precedent whose record is OUT of scope is NOT injected, so
              // the scope filter stays authoritative (no cross-group leak via few-shot).
              if (rec) { const j = rec.toJSON ? rec.toJSON() : rec; const tk = String(j[pk]); if (scopeTks && !scopeTks.has(tk)) continue; if (!scored.some((s) => s.tk === tk)) { scored.push({ tk, sim: 1 }); recByTk[tk] = j; } }
            }
          }
        }
