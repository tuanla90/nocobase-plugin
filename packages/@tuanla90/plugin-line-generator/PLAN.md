# PLAN — @tuanla90/plugin-line-generator
...
- Dùng **@tuanla90/shared settings-kit** (SettingsGrid / fi / CollapsibleSection; `rx()` KHÔNG dùng `{{$deps}}`) + **field-picker của shared** cho sourceCollection / assoc / ruleCollection / matchMap / groupBy / targetPath.
...
- Nút "AI viết hộ" cho ô công thức — tái dùng `AiCodegenButton` (@tuanla90/shared) như computed-field đã làm.
...
`evaluateFormula` hiện nằm ở `plugin-formula/src/shared/formulaEngine.ts` (thuần TS, không phụ thuộc lane). Phương án: **extract sang `@tuanla90/shared`** (module `formula-engine`), plugin-formula và plugin-line-generator cùng import — đúng rule R2 (reuse shared, không copy-paste). Sau extract: rebuild shared + rebuild cả 2 plugin consumer.
Fallback nếu extract tốn: v0 import trực tiếp từ dist của `@tuanla90/plugin-formula` (dependency), extract ở P1.
...
2. Deploy vào `node_modules/@tuanla90` (KHÔNG storage/plugins) → `pm2 restart index`.
