...
bash packages/@tuanla90/plugin-line-generator/seed/deploy-nb-local.sh  # copy vào node_modules + pm2 restart
```
Lần đầu: enable `@tuanla90/plugin-line-generator` (Plugin Manager UI upload tgz → Enable, hoặc CLI `pm add`+`pm enable`) → hard refresh. Khi enable, plugin tự tạo bảng `ptdl_linegen_rules` (`.sync()` trong load()).
