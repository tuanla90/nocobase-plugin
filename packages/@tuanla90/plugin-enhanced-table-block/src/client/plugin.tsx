...
export class PluginEnhancedTableBlockClient extends Plugin {
  async load() {
    this.app.i18n.addResources('zh-CN', '@tuanla90/plugin-enhanced-table-block/client', zhCN);
    this.app.i18n.addResources('en-US', '@tuanla90/plugin-enhanced-table-block/client', enUS);

    setEnhancedTableDeps({ useAPIClient, useTableBlockContext, useCollection_deprecated });
    try { registerFlowComponentsOnce((this as any).flowEngine?.flowSettings, { EtRespSwitch, EtRespNum }); } catch (e) { /* optional */ }
    ...
    this.app.schemaInitializerManager.addItem('page:addBlock', 'dataBlocks.enhancedTable', {
      title: '{{t("Enhanced Table", { ns: "@tuanla90/plugin-enhanced-table-block/client" })}}',
    ...
    this.app.schemaSettingsManager.addItem('blockSettings:table', 'summaryConfig', {
      ...
      useComponentProps() {
        const { t } = useTranslation(['@tuanla90/plugin-enhanced-table-block/client', 'client'], {
          nsMode: 'fallback',
        });
        const fieldSchema = useFieldSchema();
