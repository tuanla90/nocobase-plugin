# @tuanla90/plugin-field-order — Reorder collection fields
...
1. **`app.addProvider` providers do NOT render on `/admin/settings/*`** (the Collection Manager's
   ...
   externalized by the NocoBase builder — see `@tuanla90/plugin-custom-icons`).
...
- Build: `bash build-env/recipes/run-field-order-build.sh` → `storage/tar/@tuanla90/plugin-field-order-<v>.tgz`
  (markers injected).
- Deploy (nb-local): extract tgz into `node_modules/@tuanla90/plugin-field-order`, ensure the
