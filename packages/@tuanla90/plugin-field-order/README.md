...
- 🆕 **(0.1.1) The Settings-page menu is auto-tidied into groups.** The **@tuanla90** plugin settings entries are grouped and placed **right below** NocoBase's built-in ones — instead of scattered in load order. **Automatic, nothing to click.**
...
This is **automatic** — just enable the plugin, there's no button. In **/v/ → ⚙ Settings**, the **@tuanla90** settings entries are grouped for easy scanning, in this order:
...
> 💡 NocoBase's **built-in** entries (System settings, Plugin manager…) still sit **on top**; the @tuanla90 group goes **right below**. Applies to **/v/ (modern)**.
