...
const HELP_GROUPS: Array<[string, string]> = [
  ['Văn bản', 'CONCATENATE · TEXTJOIN(sep,bỏ_ô_trống,…) · LEFT · RIGHT · MID · UPPER · LOWER · PROPER · TRIM · LEN · SUBSTITUTE · TEXT · REPT · SPLIT · CONTAINS · STARTSWITH · ENDSWITH · INITIALS'],
  ['Regex', 'REGEXMATCH(text,"mẫu") · REGEXEXTRACT(text,"[0-9]+") · REGEXREPLACE(text,"mẫu","thay"). Lưu ý: \\d \\w \\s phải nhân đôi → "\\\\d+"'],
  ['Danh sách/mảng', 'LIST(a,b,…) · UNIQUE(mảng) · DISTINCT · INTERSECT(a,b) · ANY(mảng) · IN(x,mảng) · SPLIT(text,sep) — đếm phần tử bằng COUNTA'],
  ...
  ['Ngày', 'TODAY · NOW · DATE · YEAR · MONTH · DAY · DATEDIF · EDATE · ADDDAYS · ADDMONTHS · ADDYEARS · DATEADD(date,n,"day/month/year") · TEXT(date,"dd/mm/yyyy")'],
