...
Feature thêm vào `@tuanla90/plugin-formula` (2026-07-14). **Đủ 3 loại dep: local + aggregate + lookup** —
...
8. **`escapeHtml` INLINE** trong formulaEngine (server-build chỉ bundle `main` → subpath `@tuanla90/shared/format` thiếu runtime).
...
# deploy: giải nén tgz (tar --force-local) → nb-local/node_modules/@tuanla90/plugin-formula/ (+ storage/plugins)
