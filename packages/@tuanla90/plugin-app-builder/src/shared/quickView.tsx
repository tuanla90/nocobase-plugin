...
// interfaces and @tuanla90/field-enhancements `isDefault` overrides (color/icon/etc.) are honored; these
...
function makeResolver(engine: any, collection: any) {
  return (fieldPath: string, kind: Kind, componentLabel?: string): string => {
    ...
    if (isToMany) {
      // Form → prefer @tuanla90 Sub-table Pro (user's enhanced widget), fall back to the native inline
...
/** Find the collection's @tuanla90/plugin-status-flow field (if any), so generated pages can offer a
...
/** "Change status" action — @tuanla90/plugin-status-flow's own StatusTransitionActionModel (a record-scene
 ...
 *  @tuanla90/plugin-status-flow installed just doesn't get the button. */
...
export async function createQuickPage(app: any, params: CreateQuickPageParams): Promise<{ pageSchemaUid: string }> {
  ...
  const buildCols = (list: QuickColumn[]): BuiltCol[] =>
    (list || []).map((c) => {
      ...
      // component (@tuanla90/field-enhancements) — fall back to whatever the resolver picked (Dropdown select).
