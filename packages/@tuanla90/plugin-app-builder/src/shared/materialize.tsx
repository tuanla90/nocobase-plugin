 ...
 *  `{ptdlMenuKind:'divider'}` turns a CHILDLESS group into a @tuanla90/plugin-menu-enhancements section label.
...
export async function materializeApp(app: any, spec: AppSpec, onProgress?: (label: string, phase?: 'page' | 'dashboard') => void): Promise<MaterializeResult> {
  ...
  // where each spec group becomes a @tuanla90/plugin-menu-enhancements "divider" section LABEL (a childless group
  ...
  // Section style depends on @tuanla90/plugin-menu-enhancements: if enabled, each spec group renders as a
  ...
  const hasMenuEnh = (() => { try { return !!app?.pm?.get?.('@tuanla90/plugin-menu-enhancements'); } catch { return false; } })();
