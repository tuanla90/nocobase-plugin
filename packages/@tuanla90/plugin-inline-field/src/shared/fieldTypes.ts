 ...
 * bundle. The mappings are grounded VERBATIM in @tuanla90/plugin-app-builder's `fieldDef` + `relationDef`
 * (packages/@tuanla90/plugin-app-builder/src/server/plugin.ts). Supported: scalars, a computed/formula column
 * (via @tuanla90/plugin-formula) and to-one/to-many relations (m2o/o2m/m2m). o2o and statusFlow are out of scope.
 */
...
export interface InlineFieldSpec {
   ...
   *  as a @tuanla90/plugin-formula rule server-side. */
  expression?: string;
...
export function buildFieldDef(f: InlineFieldSpec): any {
  ...
  switch (f.interface) {
    ...
    case 'statusFlow': {
      ...
      // read by @tuanla90/plugin-status-flow (Tag colours + transition enforcement). Grounded in app-builder's
    ...
    case 'computed': {
      ...
      // @tuanla90/plugin-formula rule (created server-side) — so mark it read-only (x-read-pretty). Grounded
 ...
 * only: the client resolves via the framework's own `getDefaultBindingByField` first (so @tuanla90 field
