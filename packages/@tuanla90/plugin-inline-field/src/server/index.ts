 ...
 * Collection Manager. Computed → a @tuanla90/plugin-formula rule; o2m → a reverse belongsTo on the child.
 *
 ...
 *
 * ACL: `loggedIn` — matches @tuanla90/plugin-app-builder and @tuanla90/plugin-field-order (the entry point itself
    ...
    this.app.resourcer.define({
      ...
      actions: {
        createField: async (ctx: any, next: any) => {
          ...
          // Computed field → materialize a @tuanla90/plugin-formula rule (recomputes on create/update/source).
