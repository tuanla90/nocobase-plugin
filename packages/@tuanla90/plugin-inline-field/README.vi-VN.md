...
- **Luồng trạng thái** — khai báo **đơn giản**: liệt kê các trạng thái + chọn **màu** cho mỗi cái. **Trạng thái đầu = bắt đầu (init), cuối = kết thúc (end)**; chuyển tuần tự theo thứ tự. Hiển thị dạng thẻ màu + đổi trạng thái (đủ nhất khi có **@tuanla90/plugin-status-flow**).
...
> ⚠️ Cột tự tính (và nút Xem thử) cần cài **@tuanla90/plugin-formula (Công thức)**. Nếu chưa cài, cột vẫn được
...
`@tuanla90/plugin-conditional-format`. Tạo field qua action server **`ptdlInlineField:createField`**: ghi
