...
import { ColorField, getFields, IconByKey, RegistryIconPicker, setIconRegistry, SettingsGrid, rx, onLiveRefresh, SegmentedGroup, registerFlowComponentsOnce } from '@tuanla90/shared';

/**
 * @tuanla90/plugin-filter-tree — AppSheet-style group-by filter BLOCKS (two skins, one engine).
 *
...
export const NS = '@tuanla90/plugin-filter-tree/client';
...
// Collection fields fetch + per-(ds:collection) cache now comes from @tuanla90/shared `getFields`
...
const PtdlMetricField = observer((props: any) => {
  ...
  React.useEffect(() => {
    ...
    Promise.all([
      ...
      // Which of this collection's fields are @tuanla90 computed columns (roll-up/formula stored value).
...
// ---- icon (from the shared @tuanla90/plugin-custom-icons registry — consumer, no bundling) ----------
// IconByKey + RegistryIconPicker now come from @tuanla90/shared (registry wired via setIconRegistry in
...
export function registerFilterTree(deps: { flowEngine: any; api?: any; app?: any; Icon?: any; icons?: Map<string, any>; tExpr?: (s: string, o?: any) => any }) {
  ...
  try {
    registerFlowComponentsOnce(flowEngine.flowSettings, { PtdlTreeCollection, PtdlTreeField, PtdlMetricField, PtdlValueStyles, PtdlColorField: ColorField, PtdlScopeBuilder, PtdlTabs, PtdlGrid: SettingsGrid, PtdlSearchFields, PtdlPreview, InputNumber });
  } catch (e) {
    /* ignore */
  }
  try {
