...
> Install **one plugin**, then install & update **every other @ptdl plugin** from it — no browser `.tgz`
> upload, no server shell. Point it at a **manifest URL**, click a button.

**Group:** UI · **Runs on:** /admin (classic) + /v/ (modern) · **Version:** 0.1.0
...
1. **Source** — the **Manifest URL** field (default = the public @ptdl repo's `latest/index.json`), a JSON
