/**
 * @ptdl/plugin-spreadsheet-view — SPIKE (MVP bước 1, xem docs/MVP-spreadsheet-view.md).
 *
 ...
 *
 * Field đã gán widget @ptdl (Star/Progress/RichSelect/Boolean…) tự resolve ra đúng widget đó
...
export const NS = '@ptdl/plugin-spreadsheet-view/client';
...
// Widget hiển thị của @ptdl/plugin-field-enhancements dùng lại trong ô (opt-in per-cột, config bằng
...
// ---------------- formula view (engine từ @ptdl/plugin-formula qua globalThis.__ptdlFormula) ----------------
...
function evalViewFormula(formula: string, row: any, coll?: any): any {
  ...
  if (!lib?.evaluateFormula) return t('⚠ cần @ptdl/plugin-formula');
...
/**
 * Widget cell từ @ptdl/plugin-field-enhancements (bridge __ptdlFieldEnh — view component thuần React,
...
// @ptdl/plugin-formula. Bấm 1 tên → chèn NAME() + đặt con trỏ TRONG ngoặc.
 ...
 * [Mẫu] (nạp ví dụ) + [✨ AI viết hộ] (tái dùng AiCodegenButton của @ptdl/shared → endpoint
...
export function registerSpreadsheet({ flowEngine }: { flowEngine: any }) {
  ...
  // House style dialog cấu hình: SettingsGrid + CollapsibleSection của @ptdl/shared (idempotent — các
