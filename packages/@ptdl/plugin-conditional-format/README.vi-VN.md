...
Thuần client, chỉ lane `/v/`: patch `TableBlockModel.getColumns()` → bọc `onCell` (style ô: màu/nền/đậm/nghiêng/viền) và bọc `render` (chèn icon trước nội dung), **crash-safe** (mọi lỗi rơi về hành vi gốc). Quy tắc lưu trong prop khối `ptdlCondRules` qua `setProps` (MobX reactive → bảng tự re-render, không lưu vào dữ liệu). Bộ điều kiện dùng lại condition-kit của `@ptdl/shared` (`evalConditionOp`, field picker xuyên quan hệ); nội suy màu heatmap + min/max tính trên các dòng đang tải. Lane classic là no-op (không có `TableBlockModel`); server để trống. Chi tiết: `src/shared/tableRulesModel.tsx`.
