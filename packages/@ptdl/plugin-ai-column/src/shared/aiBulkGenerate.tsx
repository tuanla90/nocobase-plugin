...
/**
 * @ptdl/plugin-ai-column — "Bulk AI Generate": a COLLECTION-scene table action (appears in the
...
export function registerBulkGenerate({ flowEngine, ActionModel, ActionSceneEnum, api, tExpr }: BulkGenerateDeps) {
  ...
  try {
    flowEngine.flowSettings?.registerComponents?.({ PtdlBulkTargetField, PtdlBulkPromptInput });
  } catch (e) {
    // eslint-disable-next-line no-console
