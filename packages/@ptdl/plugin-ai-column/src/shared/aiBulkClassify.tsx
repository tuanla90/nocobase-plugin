...
/**
 * @ptdl/plugin-ai-column — "Bulk AI Classify": a COLLECTION-scene table action that, per selected
...
export function registerBulkClassify({ flowEngine, ActionModel, ActionSceneEnum, api, tExpr }: BulkClassifyDeps) {
  ...
  try {
    flowEngine.flowSettings?.registerComponents?.({ PtdlBulkClassifySource, PtdlBulkClassifyTarget });
  } catch (e) {
    // eslint-disable-next-line no-console
