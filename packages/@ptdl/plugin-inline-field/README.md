...
- **Status flow** — a **simple** config: list the states and pick a **colour** for each. **First state = start (init), last = end**; a linear flow is auto-derived. Renders as coloured tags + a change-status action (fullest with **@ptdl/plugin-status-flow** installed).
...
The plugin creates a read-only field plus a **@ptdl/plugin-formula** rule, so the value **auto-recomputes**
on create/update.

> ⚠️ Computed columns (and Preview) require **@ptdl/plugin-formula**. Without it the column is still
...
`@ptdl/plugin-conditional-format`. Field creation goes through the server action
