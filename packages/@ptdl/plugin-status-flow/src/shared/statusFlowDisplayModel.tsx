...
export const StatusFlowCellExtras: React.FC<{
  ...
  const doTransition = async (target: string) => {
    ...
    try {
      await model.context?.api?.request({
        ...
        // Tell @ptdl/plugin-change-log (if installed) this was a quick-transition tag. Header
  ...
  // Change-log history popover (only if @ptdl/plugin-change-log is installed — via its bridge
