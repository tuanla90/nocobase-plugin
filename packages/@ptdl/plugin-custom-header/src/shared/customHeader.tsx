...
/**
 * @ptdl/plugin-custom-header — STANDALONE. Ships ZERO icon library.
 *
 ...
 * read it as a string and call `.trim()` (core onCell, @ptdl enhanced-table columnTitles/summary).
 ...
 * @ptdl/plugin-custom-icons installed; no icon library bundled here. This file imports NOTHING from
...
// @ptdl/plugin-enhanced-table-block. A missing key falls back to the key text (English), so an
// incomplete locale never breaks the UI.
const CH_NS = '@ptdl/plugin-custom-header/client';
...
// ResetButton now comes from @ptdl/shared: same behavior — size="small", label "Reset",
...
// Registry-based icon picker (RegistryIconPicker) + IconByKey now come from @ptdl/shared.
...
// local default; shared fi defaults to 6) via decoratorProps. The fragment shape lives in @ptdl/shared.
...
// accordion layout other @ptdl settings dialogs use (e.g. layout-containers "Colors"/"Layout").
...
// Wire the host Icon component + icons Map into the shared registry (@ptdl/shared).
