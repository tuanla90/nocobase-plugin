...
#   - copies the tar's dist + root markers into nb-local/node_modules/@ptdl/plugin-line-generator
#   - pm2 restart index
...
TGZ=$(find "$ROOT/build-env/storage/tar/@ptdl" -name 'plugin-line-generator-*.tgz' | sort -V | tail -1)
...
echo "Done. If first install: enable @ptdl/plugin-line-generator (Plugin Manager UI or CLI), then hard-refresh."
