...
Feature thêm vào `@ptdl/plugin-formula` (2026-07-13). Đây là **lane SERVER đầu tiên** của plugin
...
- **@ptdl formula column (client)**: aggregate to-many được (auto-pluck `data.order_ids.amount`) nhưng
...
- **Debounce/batch** khi bulk create/update nhiều con cùng lúc (mỗi con hiện recompute cha 1 lần →
  N lần cho N con cùng 1 cha). Tham khảo pattern debounce của `@ptdl/plugin-gsheet-sync` writeback.
...
- **Title field toàn app** (quan hệ/picker/popup): tạo **built-in Formula field** (engine `formula.js` =
  ...
  `ptdl_label_test`.)
- **Hiển thị 1 cột (client, live)**: JS item / @ptdl formula column. JS item **PHẢI** `ctx.render(<span>{v}</span>)`
  (KHÔNG `return`), và engine @ptdl tham chiếu field qua **`data.status` / `data.name`** (không bare — vì
...
```bash
cd build-env
bash recipes/run-formula-build.sh        # tự sync src từ packages/@ptdl → build 3 lane (client/client-v2/server)
bash recipes/add-markers.sh storage/tar/@ptdl/plugin-formula-0.1.0.tgz
# deploy: giải nén tgz vào node_modules/@ptdl/ + storage/plugins/@ptdl/ (tar --force-local)
