...
Build with `bash build-env/recipes/run-layout-containers-build.sh`; deploy = extract the tgz into `nb-local/node_modules/@ptdl/plugin-layout-containers` + `storage/plugins/...` with the `applicationPlugins` row enabled.
