...
// Import from the `/format` subpath (pure utils, NO @formily) — the bare '@ptdl/shared'
// index pulls ColorField → @formily, which then has to be bundled here for no reason.
import { escapeHtml } from '@ptdl/shared/format';
...
const PAGEDJS_URL = '/static/plugins/@ptdl/plugin-print-template/dist/paged.polyfill.min.js';
