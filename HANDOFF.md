# @ptdl plugins — HANDOFF (đọc trước khi tiếp tục ở session mới)
...
## 1. `@ptdl/shared` là gì (đã dựng + chạy thật)
`packages/@ptdl/shared/` = **source lib được BUNDLE thẳng vào từng plugin** (KHÔNG phải runtime dep → plugin
...
- `format.ts` (+ subpath `@ptdl/shared/format`, pure): `get`, `formatNumber`, `formatDate`, `escapeHtml`, `interpolate`…
...
sang `build-env/node_modules/@ptdl/shared`** (consumer build resolve từ đó).

**Hành vi bundle:** client lane (rspack) **inline** shared; server lane (tsup) **vendored** vào
`dist/node_modules/@ptdl/shared` — cả hai self-contained. Mỗi plugin có **bản shared riêng** → icon-registry
...
- **settings-kit** — ✅ **KIT XONG (audit 3 subagent 2026-07-13, `docs/SETTINGS-KIT.md`)**. Nhận ra **2 lane**: (A) Formily uiSchema (đã có kit) + (B) plain-React settings page (chưa có → tự chế nhiều). Đã thêm export mới vào `@ptdl/shared/settingsKit.tsx` (additive, không phá consumer, rebuild shared xong): (A) `SEG_PROPS`, `registerSettingsKit`, `livePreview`/`previewField`, `colorStrip`, `SettingsGrid` responsive (`minColWidth`); (B) `Hint`, `SettingRow`, `ControlGrid`, `SettingCard`, `SaveBar`, `PreviewPane`. **CHƯA migrate plugin nào** (kit "cho sau này" — plugin cũ vẫn chạy bản tự chế).
...
- ✅ (2026-07-13) `plugin-block-tabs` → **`plugin-layout-containers`** — HOÀN THÀNH: Tabs + **Collapse/Sections**
  ...
  - **(round 2)** Redesign config dialog (Tab block + page + Collapse): Style+Position lên 1 hàng đầu, 5 ô màu dồn
    **1 dải swatch** (`ColorField` shared GIỮ NGUYÊN — chỉ đặt `x-component-props:{showText:false}` + `maxColumns:5` tại
    chỗ dùng, KHÔNG sửa `@ptdl/shared`), Colors/Layout thành CollapsibleSection mở sẵn. **Card giờ "dính" container**:
...
- Tooling `check-stale` (sharedHash) + `lerna run build --since` (đồ thị qua devDep `@ptdl/shared`).

## 4. Cách build + deploy (localhost nb-local = D:\...\nb-local, pm2 app `index`)
```bash
cd build-env
bash recipes/run-shared-build.sh                      # khi sửa @ptdl/shared → build + sync
bash recipes/run-<plugin>-build.sh                    # build 1 plugin (stage src + mkstub + tar)
bash recipes/add-markers.sh storage/tar/@ptdl/<pkg>-<ver>.tgz   # BẮT BUỘC (client-v2 /v/)
# deploy: giải nén tgz vào CẢ node_modules/@ptdl/<p> VÀ storage/plugins/@ptdl/<p> (tar --force-local)
cd ../../nb-local && npx pm2 restart index            # server-lane đổi mới cần restart
```
Verify: `curl .../static/plugins/@ptdl/<p>/dist/client-v2/index.js | grep <marker>`; app up = `/api/app:getInfo` 200.

**Sau khi sửa `@ptdl/shared` → `bash recipes/check-stale.sh`**: liệt kê plugin nào có tgz cũ hơn shared/dist (hoặc src đổi)
...
- Recipe "cũ" (run-condfmt/login/pwa) từng hỏng: `ROOT` thiếu `/..`, không stage src, **PKG sai scope**
  (`@taichuy`, `@nocobase/plugin-pwa`). Đã sửa về chuẩn new-style (stage từ `../packages/@ptdl/<p>`). Nếu gặp
...
- **Externalize theo import trong src (client/rspack):** nocobase-build chỉ externalize package **được import trong
  SOURCE của plugin** (không phải qua node_modules). Nếu plugin dùng antd CHỈ qua `@ptdl/shared` mà src của nó không
...
- **Consumer CHỈ dùng helper thuần** (format) nên import từ **subpath `@ptdl/shared/format`** (KHÔNG phải barrel
  `@ptdl/shared`) — barrel kéo colorField/icons/fieldPicker (antd). Tree-shake barrel KHÔNG chắc (enhanced-table được,
...
- **UI:** field = **antd `Cascader` thật** (không phải chip tự vẽ → khớp size 32px control khác) + lazy `buildLevelOptions`;
  operator/value từ `@ptdl/shared/condition`; icon `RegistryIconPicker`; toàn bộ **tiếng Việt**. apiClient inject qua
